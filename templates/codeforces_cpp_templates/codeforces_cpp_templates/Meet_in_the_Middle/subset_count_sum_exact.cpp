...
ll count_subset_sum_exact(const vector<ll>& a, ll target, bool non_empty = false) {
    int n = (int)a.size();
    vector<ll> left_sums;
    ...
    ll count = 0;
    for (ll x : left_sums) {
        auto range = equal_range(right_sums.begin(), right_sums.end(), target - x);
        count += range.second - range.first;
    }

    if (non_empty && target == 0) --count;
    return count;
}
